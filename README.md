...
![Native Demo 仪表盘](docs/image/home.png)
...
| 文件              | 用途                                                                                        |
| ----------------- | ------------------------------------------------------------------------------------------- |
| `assets/icon.png` | 跨平台源图；Native SDK 用它生成 macOS、Linux 等平台需要的图标尺寸，也方便前端或运行时读取。 |
| `assets/icon.ico` | Windows 专用多尺寸图标；嵌入 exe 后用于任务栏、窗口标题栏和 Windows 文件资源管理器。        |

因为项目同时支持 Windows、macOS 和 Linux，所以两个文件都需要保留。更换应用图标时先更新 PNG，再重新生成 ICO，并同步检查 `app.json` 的 `icons` 列表。
